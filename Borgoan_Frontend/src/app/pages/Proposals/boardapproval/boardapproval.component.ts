...
export class BoardapprovalComponent implements OnInit {
  ...
  optionList = [
    ...
    {
      label: 'Vijay Khadakbhavi. (General Manager)',
      value: 'A',
    },
    // {
    //   label: 'Ravindra C. Chougala (General Manager)',
  ...
  calcInstallments1(event) {
    ...
    if (this.data.SANCTION_AMOUNT == null || this.data.SANCTION_AMOUNT == 0) {
      ...
      if (this.browserLang == 'en') {
        ...
        this.data.LOAN_AMOUNT_IN_WORDSS = toWords_kn.convert(this.data.SANCTION_AMOUNT, { currency: true });
      }
      else if (this.browserLang == 'mr') {
        this.data.LOAN_AMOUNT_IN_WORDSS = toWords_mr.convert(this.data.SANCTION_AMOUNT, { currency: true });
      }
      console.log(this.data.LOAN_AMOUNT_IN_WORDSS, " this.data3.LOAN_AMOUNT_IN_WORDS")
    }
    this.claculate();
